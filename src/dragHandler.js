// This block provides the drag and snap functionality for the UI panel.
// It relies on the interactjs library (window.interact).
// In a modular setup, this could be a 'dragHandler.js' module.

// Assumes 'panelElement', 'config', 'updatePanelVisibility', and GM_log are accessible.
// Assumes window.interact is available (loaded via @require).

/* --- Drag Functionality (Adapted) --- */
function initDrag(panelEl) {
    if (!window.interact || !panelEl) {
        GM_log("Pack Filler Pro: interactjs library not available or panel not found. Drag functionality disabled.");
        return;
    }
...
                    updatePanelVisibility(config.panelVisible, snappedPos); // Assumes updatePanelVisibility is accessible
                    GM_log(`Pack Filler Pro: Panel drag ended. Position: ${JSON.stringify(config.panelPos)}`); // Assumes 'config' is accessible
                }
            }
        }).allowFrom('.pfp-header');
    }

// Assumes initDrag will be called during initialization.
