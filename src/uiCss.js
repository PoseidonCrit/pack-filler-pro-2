...
// Wrapped in an IIFE and attaches constants/function to window.pfpMinimal.
...
(function() {
    ...
    const MODE_SELECT_ID = window.pfpMinimal.MODE_SELECT_ID;
    const COUNT_INPUT_ID = window.pfpMinimal.COUNT_INPUT_ID;
    const FIXED_INPUT_ID = window.pfpMinimal.FIXED_INPUT_ID;
    const MIN_INPUT_ID = window.pfpMinimal.MIN_INPUT_ID;
    const MAX_INPUT_ID = window.pfpMinimal.MAX_INPUT_ID;
    const CLEAR_INPUTS_CHECKBOX_ID = window.pfpMinimal.CLEAR_INPUTS_CHECKBOX_ID;
    const DARK_MODE_CHECKBOX_ID = window.pfpMinimal.DARK_MODE_CHECKBOX_ID;
    const FILL_PACKS_BTN_ID = window.pfpMinimal.FILL_PACKS_BTN_ID;
    const CLEAR_ALL_BTN_ID = window.pfpMinimal.CLEAR_ALL_BTN_ID;
    const GM_addStyle = window.GM_addStyle; // Get GM_addStyle from window
    const GM_info = window.GM_info; // Get GM_info from window (for version)
    const GM_log = window.GM_log; // Get GM_log from window


    /* --- UI Panel HTML --- */
    // Define the minimal HTML structure for the UI panel.
    // Access via window.pfpMinimal.panelHTML
    window.pfpMinimal.panelHTML = `
...
    `;
    ...
    // Access via window.pfpMinimal.panelToggleHTML
    window.pfpMinimal.panelToggleHTML = `
...
    `;
    ...
    // Access via window.pfpMinimal.addPanelCSS
    function addPanelCSS() {
        if (typeof GM_addStyle === 'undefined') {
            if (typeof GM_log === 'function') GM_log("Pack Filler Pro: GM_addStyle function not available. Cannot inject CSS.");
            return;
        }
    ...
            }
             ...
             /* Pattern params div and single random fill button are not in minimal HTML */
             /* #pfp-pattern-params, #${FILL_RANDOM_BTN_ID} { display: none !important; } */
    ...
        `);
        if (typeof GM_log === 'function') GM_log("Pack Filler Pro: UI CSS added.");
    }

    // Attach constants and function to the global namespace object
    window.pfpMinimal.panelHTML = panelHTML;
    window.pfpMinimal.panelToggleHTML = panelToggleHTML;
    window.pfpMinimal.addPanelCSS = addPanelCSS;
